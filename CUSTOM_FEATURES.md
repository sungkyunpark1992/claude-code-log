...
기본은 **살아있는 사슬만**이다 — 모델이 실제로 기억하는 범위이므로, 새 세션에
옮겨 붙일 목적이라면 이쪽이 맞다.

켜면 **파일에 남은 모든 문답**을 담는다. 압축으로 잘려나간 앞부분과 갈라진 가지까지
전부다. 읽기·보관용이다.

```
dabc0001   기본(사슬만)  473건
           모두 담기     4,657건   ← 새 세션에 붙일 수 없는 크기
```

사슬 밖에 남은 것이 있으면 머리말에 `파일에 문답 4,184건이 더 있음` 으로 알린다.
