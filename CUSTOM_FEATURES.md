...
⏱ 세션 유지용 자동 메시지
  ...
  │ 남은 횟수 12/12 · 주기 58:00 · 다음 17:45  [⏸][⏹][↺ 횟수 초기화][전송 시늉][삭제] │
  │ ─────────────────────────────────────────────────────────── │
  │ 최근 프롬프트별 소비 토큰 [목업] : 1회당 …    ┌──────────────┐ │
  │   직전 1  이전 대화 다시 읽기 728K + 5K       │  VS Code     │ │
  │   직전 2  …                                  │  재실행 필요  │ │
  │   직전 3  …                                  └──────────────┘ │
  └───────────────────────────────────────────────────────────────┘
```
...
> 지금은 세션 크기(메시지 수)에 비례한 **목업 값**이다. 서버 API 가 붙으면 JSONL 의
> `usage` 를 읽어 실제값으로 바뀐다 — 그 값은 이미 JSONL 에 기록돼 있어 읽는 데
> 비용이 들지 않는다.
...
등록·검증·카운트다운·상태 전환. 설정은 `localStorage`(`ccl:keepalive`)에 둔다 —
서버 API 가 붙으면 파일로 옮긴다.
...
패널 마크업 + `{% include %}` 2줄. **`<details>` 가 아니라 `<div>`** 다 — 카운트다운을
늘 보고 있어야 하므로 접히면 안 된다.
...
실제 전송은 **`claude --resume <id> -p "<메시지>"`** 를 서버(파이썬)에서 돌리는 방식이다.
PowerShell 예약 작업이 아니라 서버 안에 둔다 — 사용자가 항상 `--serve` 를 켜두고,
그래야 파일 충돌·1분 단위 오차·잠금 파일이 전부 사라진다.

**전제는 이미 확인했다**([실측](#실측--cli-로-보내면-캐시가-맞는가)) — CLI 가 VS Code 의
캐시를 이어받는 것을 실제 토큰 수치로 검증했다. 남은 일은 이 명령을 서버가 대신 거는 것뿐이다.

```python
subprocess.run(["claude", "--resume", sid, "-p", msg], cwd=project_path, timeout=120)
```

`cwd` 가 중요하다 — Claude Code 는 현재 폴더를 슬러그로 바꿔 세션을 찾으므로, 그 세션의
작업 폴더에서 실행해야 한다. 값은 JSONL 의 `cwd` 필드에서 얻는다.
