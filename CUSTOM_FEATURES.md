...
- **전송 직전 mtime 확인** — 최근에 파일이 바뀌었으면 사람이 쓰는 중이므로 전송을 건너뛰고,
  카운트다운과 남은 횟수를 함께 되돌린다. 기본 설정(55분)에서는 이것만으로 대부분 막힌다.
...
#### 35-6. `components/keepalive_status.html` · `_styles.css` (신규)

세션 화면(`transcript.html`)에 끼워 넣는 상태 표시. `/api/keepalive` 를 5초마다
읽고 숫자는 매초 다시 그린다.
...
58분으로 두면 여유가 **2분**이다. 실제로 그 2분을 놓쳐 캐시가 죽었다 — 전송
예정 9분 전에 PC 가 대기모드에 들어갔고, 깨어났을 때는 이미 2시간 반이 지나 있었다.

55분으로 돌린 밤의 기록이다.

```
00:14  전송 성공 (13.0초)  캐시 남음 4.9분
01:09  전송 성공 (13.1초)  캐시 남음 5.0분
   ⋮   55분 간격으로 11회 연속
09:27  전송 성공 ( 8.2초)  캐시 남음 4.8분
```

**11회 전부 성공.** 그 사이 검사가 30초씩 두 번 멈췄는데(`★ 검사가 31초 동안
멈춰 있었다`) 5분 여유 덕에 아무 영향이 없었다. 2분이었다면 위태로웠다.

### 절전은 막을 수 없다 — 실측 2회

모던 스탠바이(S0)에 들어가면 **사용자 프로세스가 통째로 언다.** 파이썬이든
PowerShell 이든 같다. 언어 문제가 아니라 OS 가 프로세스를 멈추는 것이다.

깨우는 방법을 두 가지 시험했고 **둘 다 실패**했다.

| 방법 | 결과 |
|---|---|
| 앱이 건 타이머 (`SetWaitableTimer` fResume=True) | 예정 09:37:47 → 안 깨어남 |
| 작업 스케줄러 (`WakeToRun=True`) | 예정 10:19:38 → **10:20:17 에 39초 지각 실행** |

두 번째가 결정적이다. 작업은 실행됐지만 **직접 깨운 그 순간**이었다. 깨워서
실행한 게 아니라, 깨어나니 밀려 있던 작업이 그제야 돌아간 것이다.

프로브 기록도 같은 이야기를 한다.

```
10:14:07  ⏸ 대기 진입
10:20:16  tick 간격 371.3초  ← 구멍 371초 (= 대기 구간과 정확히 일치)
10:20:17  ▶ 대기 해제
```

AC 전원이었고 해제 타이머 정책도 「사용」이었다. S3 절전이면 됐겠지만 이 PC 는
Device Guard 가 S3 를 막아뒀다. **소프트웨어로 깨울 방법이 없다.**

남는 선택은 둘뿐이다 — 절전을 아예 막거나(전기·발열), 밤에는 포기하고 아침에
「캐시 만료 후 메시지 보내기」를 한 번 누르거나.

> 손익도 봐둘 만하다. 재작성 1회 = 읽기 12.5회이므로, 밤 8시간을 55분 주기로
> 지키면 약 9번 보낸다. 작은 세션(43K)은 **28%** 밖에 못 아끼지만 큰 세션(983K)
> 은 **83%** 를 아낀다. 작은 세션을 위해 밤새 PC 를 켜두는 건 전기값이 더 든다.

### 복제한 세션이 한 번도 전송되지 않던 문제

복제(34번)는 `sessionId` 만 바꾸고 **`cwd` 는 원본 프로젝트를 그대로 둔다.**
그 폴더가 사라지자 `claude --resume` 을 돌릴 곳이 없어 시작조차 못 했다.

```
세션 파일 위치 : C--kyo-prj-kyochon-prac
JSONL 의 cwd  : c:\kyo-prj\chicken-proj2   ← 원본, 지금은 없는 폴더
last_sent_at  : None                        ← 한 번도 성공한 적 없음
```

**기록을 고치는 방식은 택하지 않았다.** 62MB 파일에 `cwd` 가 37종 섞여 있었는데,
긴 대화가 여러 폴더를 오간 진짜 이력이라 일괄 치환하면 그게 망가진다.

대신 **읽는 쪽**을 고쳤다. 세션 파일이 놓인 폴더 이름이 곧 "이 세션이 속한
프로젝트"다 — Claude Code 가 세션을 찾는 방식 그대로다.

```python
1순위  파일이 놓인 폴더와 슬러그가 같고 실재하는 cwd
2순위  실재하는 것 중 가장 흔한 것        (대소문자만 다른 경우 등)
3순위  가장 흔한 값                      (호출자가 '폴더 없음'으로 걸러냄)
```

고친 뒤 같은 세션에서 실제 전송이 성공했다(12.0초, 파일 62,037,864 → 62,062,639).

### 진단 기록 — `keepalive-debug.log`

자리를 비운 사이 안 나갔을 때 남는 것이 "오류 문구 한 줄"뿐이면 원인을 좁힐 수
없다. 그래서 **판단의 근거**를 함께 남긴다. 기본으로 켜져 있다.

```
[dabc0001] 대기 — 만기까지 12.3분 · 캐시 남음 17.3분 · 창 5.0분
[dabc0001] ★ 작업 폴더 없음 — cwd='c:\kyo-prj\chicken-proj2' …
[dabc0001] 전송 성공 — 7.2초 걸림
★ 검사가 3000초 동안 멈춰 있었다 (정상 10초). 절전·대기모드로 얼었을 가능성
```

마지막 줄이 중요하다. 검사 간격이 벌어진 것은 **그 사이 워커가 얼어 있었다**는
뜻이라, 절전 때문인지 아닌지를 사후에 가릴 수 있는 유일한 근거다.

같은 상태는 5분에 한 줄만 남긴다 — 10초마다 전부 적으면 하룻밤에 수만 줄이 쌓여
정작 중요한 변화가 묻힌다. 2MB 를 넘으면 한 세대만 남기고 갈아끼운다.
끄려면 `CLAUDE_CODE_LOG_KEEPALIVE_DEBUG=0`.

### 세션 화면에도 상태를 — `keepalive_status`

대시보드까지 가지 않아도 대화 화면에서 바로 보이게 했다. 입력창 **바로 위**다.

```
                          54:21  [VS Code 재실행 필요]
┌──────────────────────────────────────────────────┐
│ 🤷 User                              🗑️  ▼      │
│ [ 메시지를 입력하세요... ]                        │
└──────────────────────────────────────────────────┘
...
| 일시정지 | `54:21 일시정지` 회색 |
| 캐시만료 | `00:00 캐시만료` 빨강 |
| 자동 메시지가 나간 뒤 | `VS Code 재실행 필요` 뱃지 |

등록되지 않은 세션에서는 아무것도 나오지 않는다.

#### 왜 말풍선 '안' 이 아니라 '위' 인가

처음엔 입력창 머리에 넣었는데 **누를 수 없었다.** `elementFromPoint` 로 짚어보니
원인이 둘이었다.

```css
.user.empty-prompt *  { pointer-events: none }   /* 클릭이 통과한다 */
.user.empty-prompt    { opacity: 0.4 }           /* 카운트다운이 흐리다 */
```

우선순위 싸움을 하기보다 말풍선 **밖, 바로 위**로 옮겼다. 클릭도 되고 흐려지지도
않는다. `#prompt-dock` 은 `pointer-events:none` 이라(아래 본문 클릭을 막지 않으려고)
이 줄에서만 다시 살린다.

계산식은 대시보드의 `computeState` 와 같은 규칙을 쓴다. **두 화면이 다른 숫자를
보이면 어느 쪽을 믿어야 할지 알 수 없기 때문이다.**

### 재실행 뱃지를 눌러서 지우기

뱃지를 누르면 "새로고침했다"는 뜻으로 사라진다. 상태(`needs_reload`)가 **서버에
있으므로** 어느 쪽에서 눌러도 양쪽에서 함께 사라진다.

```
세션 화면에서 클릭 → 대시보드에서도 사라짐
대시보드에서 클릭 → 세션 화면에서도 사라짐
```

동기화를 위해 따로 한 일은 없다. 상태를 서버에 둔 결과다.

### 대시보드 제목 → 대화 화면

목록의 제목을 누르면 그 세션의 대화 화면으로 간다. 주소는 이미 그려진 세션
목록에서 꺼낸다 — `.session-link` 는 `<a>` 가 아니라 `<div>` 라(수정·삭제 버튼을
담으면서 바뀌었다) 안쪽 링크에서 가져온다. 못 찾으면 링크로 만들지 않는다.
...
| **세션 화면 카운트다운 · 캐시만료 문구 · 미등록 시 숨김** | PASS |
| **뱃지 양방향 동기화 (세션↔대시보드)** | PASS |
| **제목 링크로 대화 화면 이동** | PASS |

`test/test_keepalive.py` 에 서버측 회귀 시험 21건을 뒀다.

### 실사용 실측 (2026-08-25 ~ 08-29)
...
- **모델은 100% 일치** — `claude --resume` 은 그 세션의 모델을 그대로 쓴다
- 12.79MB 세션은 재작성 한 번에 **983,212 토큰**이 들었고 그 직후 한도에 걸렸다.
  **큰 세션은 대상에서 빼는 편이 낫다**

## 36. 메시지 골라 삭제 · 되돌리기

**목적**: 자동 메시지나 실수로 보낸 질문이 기록을 어지럽힐 때 골라서 지운다.
토큰을 아끼려는 것이 아니다 — 자동 메시지는 파일의 2~3% 밖에 안 되고, 대화 중간을
지우면 그 지점부터 캐시가 무효가 되어 오히려 손해다. **읽기 좋게 만드는 것**이 목적이다.

이미 있던 **선택 모드**(☑️, 원래는 HTML 내보내기용)에 삭제를 붙였다. 새 화면을 만드는
대신 기존 것을 쓴 이유는, 고르는 방법이 둘로 갈리면 어느 쪽이 무엇을 지우는지
헷갈리기 때문이다.

```
[전체 선택] [선택 해제] [📤 HTML로 내보내기]
─────────────────────────────────────────────
[🤖 자동 메시지 선택] [🗑️ 기록에서 삭제] [↩️ 되돌리기]
```

### 지우는 단위 — 고른 것만

**질문만, 답변만, 또는 둘 다** 고를 수 있다.

| 고른 것 | 지워지는 것 |
|---|---|
| 질문 | 질문 + 그 질문에 딸린 `attachment` |
| 답변 | 답변 하나 |
| 둘 다 | 문답 한 쌍 전부 |

처음에는 무엇을 고르든 문답 한 쌍을 통째로 지웠다. 질문만 지우면 답이 홀로 남아
기록이 더 이상해진다고 봤기 때문이다. 그런데 실제로 써 보니 **한도 초과 안내문이나
`No response requested.` 같은 답변만 걷어내고 싶은 경우**가 있었다. 그래서 고른 것만
지우도록 바꾸고, 대신 답변을 잃는 질문이 생기면 확인창에서 알린다.

`attachment` 는 홀로 의미가 없어 붙어 있던 메시지와 운명을 같이한다.

### 반드시 지켜야 하는 두 가지

#### ① 단위의 끝은 '다음 대화 항목 직전'이다

Claude Code 의 JSONL 은 트리처럼 보이지만 사실상 **연결 리스트**다. 각 메시지의 부모가
바로 앞 메시지라, "이 노드의 자손을 전부 지운다"고 하면 **그 뒤 대화가 통째로 딸려
나온다.** 만들면서 실제로 겪었다 — 3줄짜리를 지우려다 22줄이 지워졌다.
...
지우기 **전에** 결과의 사슬을 미리 검사하고, 성하지 않으면 아무것도 하지 않는다.

### 안전장치

| | |
|---|---|
| **미리보기** | 무엇이 지워질지 서버에 먼저 묻고, 그 숫자를 확인창에 보인다 |
| **백업** | `…jsonl.bak-20260831-170708` 자동 생성. 되돌리기도 직전 상태를 남긴다 |
| **사슬 사전 검사** | 깨질 결과면 파일을 건드리지 않는다 |
| **파일명 확인** | 세션 ID 와 정확히 같을 때만 손댄다 (세션 삭제와 같은 장치) |
| **경로 차단** | 되돌릴 백업은 **이름만** 받고, 이 세션의 형식일 때만 연다 |

확인창은 이렇게 나온다.

```
메시지 20건 (질문 10건 + 답변 10건), 30줄을 기록에서 지웁니다.

· 고른 것만 지웁니다 — 질문을 골라도 답변은 남습니다
· 되돌릴 수 없습니다 (백업 파일은 남습니다)
· 지운 뒤 VS Code 를 새로고침해야 대화가 갈라지지 않습니다

⚠ 답변이 하나도 남지 않는 질문이 1건 생깁니다.

진행할까요?
```

마지막 경고는 **막지 않는다** — 답변만 지우고 싶을 때가 있기 때문이다. 모르고 그렇게
되는 일만 없게 한다.

### 화면에서 고르는 법

**말풍선 아무 데나 눌러도** 골라진다. 체크박스는 18px 짜리 과녁이라 하나씩 맞추기가
번거롭다. 글자를 끌어 복사하거나 링크·버튼·접기를 누르는 것은 그대로 동작한다.

**「🤖 자동 메시지 선택」** 은 서버에 등록된 자동 메시지 문구를 가져와 일치하는 질문과
**그 답변까지** 한 번에 고른다. 등록을 지운 뒤에도 기록에는 남아 있으므로, 이 세션의
설정이 없으면 다른 세션의 문구로도 찾는다.

### 만들면서 겪은 것들

#### ⓐ 클릭이 선택 모드를 꺼뜨렸다

말풍선 클릭을 선택으로 만들자 첫 클릭만 되고 두 번째부터 먹통이 됐다. 계측해 보니
**「바깥 클릭 시 패널 닫기」가 메시지 클릭까지 바깥으로 보고** 있었다.

```
1번째 클릭 후 selMode = True
2번째 클릭 후 selMode = False   ← 선택 모드가 꺼져 있었다
```

대화 영역(`.message`) 클릭은 닫지 않도록 고쳤다. 세션 머리글·접기 막대도 그 안이다.

#### ⓑ "메시지가 사라진다" — 접기였다

체크를 반복하다 마지막 답변이 사라진다는 신고가 있었다. DOM 관찰기를 붙여 잡았다.

```
숨겨짐(display:none)  id=msg-d-100  클래스= message assistant d-0 d-99 msg-selected
```

`d-99` 는 **99번 메시지의 자손**이라는 뜻이다. 접기는 `.message.d-99` 를 전부 숨기므로
질문을 접으면 답변이 딸려 숨는다. 삭제가 아니라 **접힌 것**이고 다시 누르면 돌아온다.
말풍선 전체를 누르게 되면서 접기 막대를 스치는 일이 잦아진 것이 계기였다.

#### ⓒ 세는 방식이 오해를 불렀다

`선택 (18)` 인데 안내문은 `자동 메시지 14건(답변 포함 **0개**)` 이었다. 새로 고른 것만
셌기 때문이다 — 두 번째로 누르면 전부 "이미 선택됨"이라 0이 된다. **지금 선택된 전체**
를 알리도록 고쳤다.

#### ⓓ 백업 이름이 초 단위라 겹쳤다

지우고 곧바로 되돌리면 같은 초라 이름이 겹쳐 **앞선 백업을 덮어썼다.** 되돌릴 곳이
사라지는 문제다. 일련번호를 붙여 피하고, 목록 정렬도 이름순 대신 만든 시각순으로
바꿨다 (이름순이면 `-10` 이 `-2` 보다 앞선다).

#### ⓔ heredoc 이 `\n` 을 실제 줄바꿈으로 바꿔 JS 를 죽였다

35번 「함정 ③」과 같은 문제다. 확인창 문구를 heredoc 으로 쓰다가 문자열 안에 실제
줄바꿈이 들어가 페이지 전체 JS 가 멈췄다. `String.fromCharCode(10)` 으로 우회하고,
**페이지 JS 가 살아 있는지** 확인하는 항목을 시험에 넣어 회귀를 잡도록 했다.

### 삭제 뒤 곧바로 새로고침

기다리게 하면 "아무 일도 안 났다"고 여겨 직접 새로고침하게 된다. 실제로 그런 신고가
있었다 — 1.8초 뒤 새로고침하도록 해뒀는데 그 사이를 못 기다린 것이다.

이제 서버가 확인하는 즉시 새로고침한다(실측 **0.3초**). 결과 문구는 `sessionStorage` 에
잠깐 맡겨 두었다가 새 화면에서 되살리고, 이어서 더 고를 수 있게 패널도 열어 둔다.

### 수정 파일 (2개 신규 + 4개)

| | |
|---|---|
| `message_delete.py` (신규) | 단위 계산·사슬 복구·백업·되돌리기 |
| `test/test_message_delete.py` (신규) | 회귀 시험 23건 |
| `server.py` | API 3개 (`messages/delete`, `backups`, `restore`) |
| `components/message_export.html` | 선택·삭제·되돌리기 화면 |
| `components/export_styles.css` | 위험 동작 구분, 클릭 가능 표시 |
| `transcript.html` · `messages_fragment.html` | 메시지에 `data-uuid` 부여 |

`data-uuid` 가 핵심이다. 기존 `data-message-id` 는 `d-3` 같은 **화면 순번**이라 다시 그릴
때마다 밀려 지울 대상을 특정할 수 없다.

### 검증

| 항목 | |
|---|---|
| 단위 경계 — 다음 대화 항목 직전에서 멈춤 | PASS |
| 질문만 / 답변만 / 둘 다 지우기 | PASS |
| 사슬 재연결 · 고아 0건 | PASS |
| 답변 잃는 질문 경고 | PASS |
| 건드리지 않은 줄은 글자 그대로 보존 | PASS |
| 모르는 항목·빈 선택 거부 | PASS |
| 백업 생성 · 되돌리기 · 왕복(다시 앞으로) | PASS |
| 백업 이름 겹침 방지 | PASS |
| 경로 차단 (`../../etc/passwd` 등 5종) | PASS |
| 깨진 백업으로는 되돌리지 않음 | PASS |
| 말풍선 클릭 선택/해제 · 버튼은 그대로 | PASS |
| 삭제 뒤 자동 새로고침 · 결과 유지 | PASS |

### 실사용 실측 (2026-08-31)

두 세션에서 실제로 지워 보고 매번 백업과 대조했다.

| 세션 | 지운 것 | 결과 |
|---|---|---|
| `7caad578` | 36건 (질문 18 + 답변 18) | 239 → 182줄 · 사슬 온전 |
| ↳ 되돌리기 | 18건 복원 | 대화 항목 백업과 완전 일치 |
| `23a08897` | 18건 (질문 9 + 답변 9) | 442 → 416줄 · 사슬 온전 |

**삭제로 생긴 고아는 모든 회차에서 0건**이었다.

> `ba1ea00b` 처럼 압축·분기를 겪은 세션에는 **원래부터 고아인 항목**이 있다(2건).
> 삭제가 만든 것과 구분하려면 삭제 전후를 비교해야 한다 — 절대 개수를 보면 오판한다.

---

## 37. 문답만 추려 .md 로 뽑기

**목적**: JSONL 을 복제해 대화를 이어가면 문맥에 **대화가 아닌 것**이 잔뜩 실린다.
도구 실행 결과·추론·그림을 빼고 문답 본문만 남겨 새 세션에 옮겨 붙이기 위한 것이다.

### 실측 — 무게가 어디에 있나

살아있는 사슬(실제로 API 에 실려 가는 것)의 구성이다.

| 세션 | 순수 문답 | 나머지 |
|---|---|---|
| `ba1ea00b` | **5.3%** | image 44.9% · tool_use 31.6% · tool_result 18.1% |
| `7caad578` | 13.9% | tool_use 46.0% · tool_result 40.1% |
| `23a08897` | 23.2% | tool_result 59.8% · tool_use 17.0% |
| `dabc0001` | 38.0% | tool_use 32.0% · tool_result 30.1% |

**문답만 남기면 문맥이 62~95% 줄어든다.** `ba1ea00b` 은 878K → 47K 토큰이다.

### 파일 크기로 재면 안 된다

처음에 파일 전체로 계산했더니 **216%** 같은 값이 나왔다. 말이 안 되는 값이라
따져보니 전제가 틀렸다.

```
dabc0001   파일 62MB · 대화 4,657건
           └ 실제 문맥은 473건뿐   ← 압축(compact)이 앞부분을 요약으로 대체
```

JSONL 은 **지금까지의 모든 것을 덧붙여 쌓은 기록**이고, 모델에 실려 가는 것은
마지막 메시지에서 `parentUuid` 를 거슬러 오른 **한 줄기**뿐이다. 갈라진 가지와
압축으로 잘린 앞부분은 보내지 않는다.

**파일이 커지는 것 자체가 토큰을 먹는 게 아니다.**

### 되짚을 수 있게 남기는 것

도구 결과를 버리면 "그때 왜 그렇게 결론냈지" 를 확인할 길이 사라진다. 그래서
각 문답에 **원본 uuid** 를 적고, 머리말에 찾아갈 수 있는 정보를 모아 둔다.

```markdown
| 원본 세션 ID | `7caad578-2567-46b2-b646-f58ac84fa274` |
| 원본 파일 | `C:\Users\user\.claude\projects\C--Users-user\7caad578-....jsonl` |
| 원본 화면 | http://localhost:5678/C--Users-user/session-7caad578-....html |
| 덜어낸 정도 | 17,034자 → 2,374자 (13.9%) |

## 1-1. 🤷 질문 · 2026-08-06 11:07:02

`uuid: c712551b-76f2-4273-890a-cf12f27f3541`

C:\a에 있는 폴더 내용이 뭐야?
```

번호는 **`1-1`, `1-2`** 로 매긴다. 한 질문에 답변이 여러 번 나뉘어 오는 일이 흔해서
(도구를 쓰는 사이사이), 질문 번호만으로는 어느 글인지 가리킬 수 없다.
...
대화가 길어지면 Claude Code 가 앞부분을 **요약 한 덩어리로 갈아치운다.** 그 요약문이
맨 앞에 들어가고 그 뒤의 문답이 이어진다. **지금 모델이 기억하고 있는 그대로**라,
새 세션에 옮겨 붙일 목적이라면 이쪽이다.

요약문은 `isCompactSummary` 표식이 붙은 user 항목이고 실제로 사슬의 첫머리에 있다.

```
dabc0001 사슬의 맨 앞
   [system]                    (빈 항목)
   [user] isCompactSummary     "This session is being continued from a
...
```
[질문1][답변1][질문2][답변2] … [질문300][답변300]
```

요약으로 잘려나간 앞부분과, 대화가 갈라지며 버려진 가지까지 전부 담는다.

```
dabc0001   기본(사슬만)  473건
           모두 담기     4,657건   ← 약 10배. 새 세션에 붙일 수 없는 크기
```

시각이 겹친다는 점도 알아둘 만하다. "앞부분 = 사슬 밖" 이 아니다 — 압축 이후에
갈라진 가지도 사슬 밖에 있어서, 최근 날짜까지 걸쳐 있다.

```
사슬 밖 4,319건 : 08-08 07:55 ~ 08-28 14:16
사슬      473건 : 08-25 01:26 ~ 08-29 00:27
                  └ 08-25~08-28 이 겹친다
```

그래서 「모두 담기」로 뽑으면 시간순으로 늘어놓되 **같은 시점의 다른 갈래가 나란히
나올 수 있다.** 읽기용으로는 괜찮지만 대화를 이어갈 목적으로는 혼란스럽다.

끄고 뽑으면 빠진 것을 머리말에 적어 둔다 — `파일에 문답 4,184건이 더 있음`.

#### 화면 설명은 도식으로

처음엔 한 줄로 적었는데(`기본은 살아있는 사슬만입니다 — 모델이 실제로 기억하는
범위죠…`) 무슨 말인지 알 수 없어 매번 다시 묻게 됐다. 켰을 때와 껐을 때 **무엇이
담기는지** 를 도식으로 나란히 두는 편이 훨씬 낫다.
...
도식 줄은 고정폭 글꼴에 `overflow-x: auto` 를 준다 — 길어져도 페이지 전체가 옆으로
밀리면 안 된다. 시험에서 그것까지 확인한다.

### 원본 JSONL 옆에 저장한다

```
7caad578-2567-46b2-b646-f58ac84fa274.jsonl
7caad578-2567-46b2-b646-f58ac84fa274-대화만.md
```

브라우저 다운로드 폴더로 흩어지면 어느 세션에서 뽑았는지 알 수 없고, 원본과 함께
있어야 uuid 로 되짚기도 쉽다. 같은 세션을 다시 뽑으면 **덮어쓴다** — 파생물이고
원본은 그대로다.
...
| | |
|---|---|
| 대시보드 `📝 문답만 추려 내보내기 (.md)` | 어느 세션이든 골라서. ID·앞 8자리·경로 모두 인식 |
| 세션 화면 선택 패널 `📝 문답만 추려 내보내기` | 보던 대화를 바로 |

두 곳에서 뽑은 파일이 글자까지 같은지 시험으로 확인한다.

### 알아둘 것 — 잃는 것이 분명하다

잘라내는 `tool_result` 가 곧 **모델이 코드베이스에 대해 아는 전부**다.

```
"아까 그 함수 다시 고쳐줘"     → 그 함수를 본 적이 없게 됨
"어제 얘기한 방식대로 진행해"  → 결론은 알지만 근거는 모름
```

**설계·의사결정 논의를 잇는 데는 충분하고, 코드 작업을 그대로 잇는 데는 부족하다.**

### 수정 파일 (3개 신규 + 4개)

`digest.py`(신규) · `components/digest_script.js`(신규) · `test/test_digest.py`(신규)
· `server.py` · `index.html` · `transcript.html` · `components/message_export.html`

### 검증

| 항목 | |
|---|---|
| 도구 호출·결과·그림이 빠짐 | PASS |
| 문답 본문은 남음 | PASS |
| 각 문답에 uuid | PASS |
| 세션 ID·원본 경로·화면 주소가 내용에 | PASS |
| 번호가 `문답-순서` 로 | PASS |
| 사슬만 담기 / 모두 담기 | PASS |
| 도구 결과만 있는 차례는 건너뜀 | PASS |
| 원본 JSONL 옆에 생성 | PASS |
| 대시보드·세션 화면 결과가 같음 | PASS |
| 선택칸 설명이 끄면/켜면 두 경우로 · 페이지가 가로로 안 밀림 | PASS |

`test/test_digest.py` 7건 · 브라우저 검증 18항목.

---

## 38. 나중에 물어볼 질문 적어두기 (💾 · 🗂️)

**목적**: 프롬프트를 쓰다 보면 "지금 말고 나중에" 물어야 할 것이 생긴다. 그때마다
다른 곳에 옮겨 적으면 **어느 세션에서 하려던 이야기였는지 잃어버린다.**

### 동작

입력창 머리의 **💾** 를 누르면 저장되고 **입력창은 비워진다.** 남겨두면 실수로 또
보내게 된다. 지우기(🗑️)와 같은 방식을 써서 `Ctrl+Z` 로 되살릴 수 있다.

떠 있는 버튼 **🗂️** (북마크 🔖 옆)로 목록을 연다. 저장 개수가 배지로 붙는다.

| 목록에서 | |
|---|---|
| **입력창에 넣기** | 되꺼내 쓴다. 저장본은 그대로 남는다 |
| 복사 | 클립보드로 |
| 삭제 | 확인 후 |
...
```
7caad578-2567-46b2-b646-f58ac84fa274.jsonl
7caad578-2567-46b2-b646-f58ac84fa274-질문메모.md
```

대화 기록을 열면 적어둔 질문도 바로 곁에 있다. 패널 위에 전체 경로를 보여 준다.

### 왜 Markdown 한 파일인가

편집기로 열어 **손으로 고칠 수 있어야** 한다. JSON 은 사람이 읽기 나쁘고, 질문마다
파일을 따로 만들면 폴더가 금세 지저분해진다.

```markdown
# 나중에 물어볼 질문 — 7caad578

- 세션 : `7caad578-2567-46b2-b646-f58ac84fa274`
- 화면 : http://localhost:5678/C--Users-user/session-7caad578-....html

<!-- ccl-draft 20260831-190412 -->
## 2026-08-31 19:04:12
...
되읽으려면 어디서 한 건이 끝나는지 알아야 한다. 그런데 **질문 본문에 `---` 나
`## ` 가 들어가는 일은 흔하다.** 사람이 우연히 칠 만한 글을 경계로 삼으면 그 순간
메모가 뭉개진다.

그래서 눈에 보이지 않는 HTML 주석(`<!-- ccl-draft ... -->`)을 쓴다. 렌더된 Markdown
에는 나타나지 않고, 사람이 실수로 칠 일도 없다.

식별자가 초 단위라 잇달아 저장하면 겹친다 — 일련번호를 붙여 피한다(백업 이름에서
겪은 것과 같은 문제다, [36번](#36-메시지-골라-삭제--되돌리기) 참고).

마지막 하나를 지우면 **빈 껍데기 파일을 남기지 않는다.**

### 함정 2가지

**① 입력창이 비어 있으면 💾 가 눌리지 않았다**

`.user.empty-prompt *` 가 `pointer-events: none` 이라 클릭이 통과한다. 눌러도 아무
일이 없으면 고장 난 것으로 보이므로, 더 구체적인 선택자로 살려 "비어 있습니다" 를
알려주게 했다. (35번 세션 화면 표시에서 겪은 것과 같은 규칙이다.)

**② 안내 문구가 곧바로 지워졌다**

저장 직후 띄운 문구를 **비동기 목록 조회가 덮어썼다.** 문구가 떠 있는 동안에는
경로를 쓰지 않도록 시각을 기억해 둔다.

### 수정 파일 (4개 신규 + 2개)

`drafts.py`(신규) · `components/prompt_drafts.html`(신규) ·
`components/prompt_drafts_styles.css`(신규) · `test/test_drafts.py`(신규) ·
`server.py` · `transcript.html`

### 검증

| 항목 | |
|---|---|
| JSONL 옆에 생성 · 이름이 세션 ID | PASS |
| **본문에 `---` · `## ` 가 있어도 온전** | PASS |
| 최근 것이 먼저 | PASS |
| 잇달아 저장해도 식별자가 안 겹침 | PASS |
| 빈 내용 거부 | PASS |
| 하나 지워도 나머지는 남음 | PASS |
| 마지막을 지우면 파일도 없앰 | PASS |
| **손으로 고친 파일도 읽힘** | PASS |
| 저장 뒤 입력창 비움 · 배지 갱신 | PASS |
| 입력창에 되돌려 넣기 | PASS |
| 새로고침 뒤에도 남음 | PASS |

`test/test_drafts.py` 10건 · 브라우저 검증 19항목.
